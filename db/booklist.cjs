...
const createBook = async (title, author, description, coverImage, available, free, rent, buy, categoryId, userId) => {
  try {
    await client.query(`
      INSERT INTO booklist (title, author, description, cover_image, available, free, rent, buy, category_id, user_id)
      VALUES ('${title}', '${author}', '${description}', '${coverImage}', ${available}, ${free}, ${rent}, ${buy}, ${categoryId}, ${userId});
    `);
  } catch (err) {
    console.log(err);
  }
